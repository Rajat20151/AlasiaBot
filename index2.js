...
bot.on('ready', async() => {
    ...
    bot.user.setActivity('You', { type: 'WATCHING'}) // or 'PLAYING' 'LISTENING' 'STREAMING'
  ...
  bot.user.setStatus('Online') // Can be 'Online' , 'idle' , 'invisible' or 'dnd' (do not disturb) 
...
bot.on('message', async message => {
 ...
 // Hello
 if (msg === prefix + 'LIST') { // This checks if msg (the message but in all caps), is the same as the prefix + the command in all caps.

    // Now, let's send a response.
    message.channel.send('Check your DMs!'); // This 'sends' the message to the channel the message was in. You can change what is in the message to whatever you want.
    message.author.send('**The following people are currently in the Support Team Management:** \n **xDesi** - *Owner* \n **Caro47** - *Director* \n **Ah** - *Head-Admin* \n *Please keep in mind these people have different jobs, please contact the right person for your problem.*')
    ...
    if (msg.startsWith(prefix + 'USERINFO')) {
		...
		.setFooter("Created by Skyverse Bot")
		.setTimestamp()
		.setDescription("----------------------------------------------------------")
		.setColor("#0000FF")
...
bot.login('NDY4MTEwMjI1OTQxOTg3MzI4.Dj0Jww.oNCADgihcojolWVQ6KtYQHpjkqU');